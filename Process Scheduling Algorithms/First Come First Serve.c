...
void heapify(schedule *s)
{
    ...
    for(i=s->numberOfProcesses/2;i>=1;i--)
    {
        ...
        {
            j=2*k;
            if(j<s->numberOfProcesses && s->processes[j].arrivalTime>s->processes[j+1].arrivalTime)
                j++;
            if(p.arrivalTime<=s->processes[j].arrivalTime)
                heap=true;
            else
            {
...
schedule firstComeFirstServe(schedule s,float *awt,float *atat)
{
    int size=0,time=0;
    process p;
    schedule s1;
...
int main()
{
    ...
    readProcesses(&s,in);
    fclose(in);
    s=firstComeFirstServe(s,&awt,&atat);
    out=fopen("Output.txt","w");
    fprintf(out,"Schedule according to First Come First Serve:-\n");
    showProcesses(s,out);
