...
#define PREDECESSOR_NOT_FOUND INT_MIN+1
#define SUCCESSOR_NOT_FOUND INT_MAX
...
void menu()
{
    ...
    printf("5.Print the predecessor of an element\n");
    printf("6.Print the successor of an element\n");
    printf("7.Exit\n");
...
int predecessor(BST tree,int ele)
{
    treeNode* cur=search(tree,ele),*pred,*temp;
    if(cur==NULL)
    ...
    while(temp!=NULL)
    {
        if(cur->element>temp->element)
        {
            pred=temp;
            temp=temp->right;
        }
        else if(cur->element<temp->element)
            temp=temp->left;
        else
            break;
    }
    return (pred==NULL)?PREDECESSOR_NOT_FOUND:pred->element;
}

int successor(BST tree,int ele)
{
    treeNode* cur=search(tree,ele),*succ,*temp;
    if(cur==NULL)
        return ELEMENT_NOT_FOUND;
    if(cur->right!=NULL)
    {
    ...
    return (succ==NULL)?SUCCESSOR_NOT_FOUND:succ->element;
}
    ...
    do
    {
        ...
        switch(choice)
        {
            ...
            case 5:printf("Enter the element:");
                    scanf("%d",&ele);
                    ele=predecessor(tree,ele);
                    if(ele==ELEMENT_NOT_FOUND)
                        printf("Element not found in the Binary Search Tree\n\n");
                    else if(ele==PREDECESSOR_NOT_FOUND)
                        printf("No predecessor found for the element\n\n");
                    else
                        printf("Predecessor of the element is %d\n\n",ele);
                    break;
            case 6:printf("Enter the element:");
                    scanf("%d",&ele);
                    ele=successor(tree,ele);
                    if(ele==ELEMENT_NOT_FOUND)
                        printf("Element not found in the Binary Search Tree\n\n");
                    else if(ele==SUCCESSOR_NOT_FOUND)
                        printf("No successor found for the element\n\n");
                    else
                        printf("Successor of the element is %d\n\n",ele);
                    break;
